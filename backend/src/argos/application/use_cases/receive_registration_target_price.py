...
REGISTRATION_TARGET_PRICE_REPLIES = RegistrationTargetPriceReplies(
    accepted="Preço-alvo {price_brl} registrado no rascunho de teste. Envie 12 ou 24 para definir o intervalo em horas.\n\nUse /cancelar para interromper o cadastro.",
...
class ReceiveTelegramRegistrationTargetPrice:
    ...
    def execute(
        ...
        if target_price_cents is not None:
            replies = replace(replies, accepted=f"Preço-alvo {format_target_price_brl(target_price_cents)} registrado no rascunho de teste. Envie 12 ou 24 para definir o intervalo em horas.\n\nUse /cancelar para interromper o cadastro.")

"""Avanço do apelido e resposta durável sob o lease da inbox."""
...
class PostgreSQLTelegramRegistrationAliasRepository:
    ...
    def receive_for_update(
        ...
        with self._engine.begin() as connection:
            ...
            try:
                verified_alias = normalize_product_alias(text)
            except ValueError:
                verified_alias = None
            if verified_alias != normalized_alias:
            ...
            if user is not None and draft is not None and draft["expires_at"] > effective_at:
                if draft["state"] != "awaiting_alias":
                    reply = replies.unexpected_state
                elif verified_alias is None:
                    reply = replies.invalid_alias
                else:
                    changed = connection.scalar(update(table).where(
                        table.telegram_user_id == telegram_user_id,
                        table.version == draft["version"],
                        table.state == "awaiting_alias",
                        table.expires_at > effective_at,
                    ).values(
                        state="awaiting_target_price", data={**draft["data"], "alias": verified_alias},

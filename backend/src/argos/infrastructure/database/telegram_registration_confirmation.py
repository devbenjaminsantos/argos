...
class PostgreSQLTelegramRegistrationConfirmationRepository:
    ...
    def confirm_for_update(
        ...
        with self._engine.begin() as connection:
            return self.confirm_in_transaction(
                ...
                observed_at=observed_at, replies=replies,
            )

    def confirm_in_transaction(
        self, *, connection: Connection, update_id: int, lease_token: UUID,
        telegram_user_id: int, chat_id: int, text: str, observed_at: datetime,
        replies: RegistrationConfirmationReplies,
    ) -> TelegramMessage:
        """Compartilha a transação da seleção do passo; não confirma sozinha."""

"""Avanço do intervalo e resposta durável sob o lease da inbox."""
...
class PostgreSQLTelegramRegistrationIntervalRepository:
    ...
    def receive_for_update(
        ...
        with self._engine.begin() as connection:
            ...
            try:
                verified_hours = parse_collection_interval_hours(text)
            except ValueError:
                verified_hours = None
            if (interval_hours is not None and (not isinstance(interval_hours, int) or isinstance(interval_hours, bool))) or verified_hours != interval_hours:
            ...
            if user is not None and draft is not None and draft["expires_at"] > effective_at:
                if draft["state"] != "awaiting_interval":
                    reply = replies.unexpected_state
                elif verified_hours is None:
                    reply = replies.invalid_interval
                else:
                    changed = connection.scalar(update(table).where(
                        table.telegram_user_id == telegram_user_id,
                        table.version == draft["version"],
                        table.state == "awaiting_interval",
                        table.expires_at > effective_at,
                    ).values(
                        state="awaiting_confirmation", data={**draft["data"], "interval_hours": verified_hours},

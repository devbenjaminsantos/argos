...
from argos.application.use_cases.receive_registration_alias import ReceiveTelegramRegistrationAlias
...
def test_passes_identity_raw_text_and_alias_and_returns_persisted_reply():
    repo = Repository()
    raw = "  Caneca   Hello Kitty  "
    assert ReceiveTelegramRegistrationAlias(repo).execute(**(_INPUT | {"text": raw})) is repo.reply
    call = repo.calls[0]
    for key in ("update_id", "lease_token", "telegram_user_id", "chat_id", "observed_at"):
        assert call[key] == _INPUT[key]
    assert call["text"] == raw
    assert call["normalized_alias"] == "Caneca Hello Kitty"


def test_invalid_alias_requests_durable_reply():
    repo = Repository()
    assert ReceiveTelegramRegistrationAlias(repo).execute(**_INPUT) is repo.reply
    assert repo.calls[0]["normalized_alias"] is None
...
def test_invalid_input_never_accesses_repository(changes):
    ...
    with pytest.raises(ApplicationError):
        ReceiveTelegramRegistrationAlias(repo).execute(**(_INPUT | changes))
    assert repo.calls == []
...
def test_lost_lease_is_propagated():
    ...
    with pytest.raises(RuntimeError, match="Lease perdido"):
        ReceiveTelegramRegistrationAlias(LostLease()).execute(**_INPUT)

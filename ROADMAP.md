...
- [x] Ligar o caso de uso interno de verificação à gravação de uma observação de sucesso ou falha: executor fornece UUID/timestamp, entrada inválida ou alvo ausente não cria histórico, e resultado só retorna após persistência. Vinte e um testes do caso de uso e duas integrações PostgreSQL do caminho completo aguardam CI.

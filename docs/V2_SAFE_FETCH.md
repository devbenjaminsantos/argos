...
Resultado do transporte preparado: fetch_html_once retorna FetchedHTML imutável, com html em bytes e final_url do destino validado que respondeu 200. HTML e URL não aparecem no repr padrão. Testes verificam destino direto, redirect relativo e troca de host; 68 testes relacionados passaram localmente. Sem alteração no bot ou tráfego externo. Próximo incremento: inspecionar o extrator V1 e selecionar fixtures locais para portar a extração Mercado Livre, mantendo falhas explícitas e preços em centavos.

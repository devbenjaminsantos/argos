...
A admissão HTTP aceita o limite de comando `/verificar` com espaço/tabulação ou sem argumento; o segundo caso é persistido para produzir orientação durável. `/produtos` exibe o UUID completo somente junto aos produtos do proprietário. `/start`, `/ajuda` e mensagens de cadastro anunciam a consulta manual e mantêm alertas automáticos como pendentes. A suíte completa local passou; CI e deploy ainda estão pendentes.

Coleta manual persistida e `/verificar` estão implementados, mas ainda aguardam CI, deploy e aceitação manual. Job periódico e alertas continuam pendentes. A aceitação funcional entre duas contas da V2.9 foi concluída por relato do usuário; não houve consulta administrativa final.

...
`PostgreSQLPriceObservationRepository` grava com `INSERT ... ON CONFLICT DO NOTHING` dentro de uma transação. Em colisão, compara todos os campos da linha vencedora: repetição idêntica retorna sem duplicar; reutilização divergente do UUID lança `PriceObservationConflictError` e preserva a linha original. Quatro integrações cobrem replay sequencial, divergência e as duas formas concorrentes; aguardam CI PostgreSQL. A ligação ao caso de uso ainda não existe.

...
Migração `20260916_10` e contrato `PriceObservation` preparados para histórico append-only. O UUID da observação é a futura chave idempotente. Sucesso exige preço entre 1 e 999999999 e fonte conhecida, sem erro; falha exige preço/fonte nulos e código sanitizado. A meta de preço é preservada como snapshot. Uma FK composta por produto/proprietário impede registros cruzados. Data API roles e PUBLIC não leem a tabela; `argos_runtime` recebe somente SELECT/INSERT. Downgrade recusa apagar histórico existente. Doze testes de contrato passaram localmente e 12 integrações PostgreSQL foram aprovadas no CI 35123926472, commit 7c34163. O repositório de escrita e a ligação ao caso de uso ainda não existem.

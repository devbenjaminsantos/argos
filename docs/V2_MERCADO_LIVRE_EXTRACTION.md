...
O contrato Telegram aceita somente `/verificar <UUID canônico completo>`, com espaços ou tabulação no mesmo texto e sem argumentos extras. O UUID da observação é derivado de forma determinística do `update_id`, permitindo que o executor preserve a chave em recuperação. A resposta de sucesso usa somente o snapshot durável de preços e a comparação com a meta; omite título remoto, apelido mutável e fonte técnica. Falhas usam uma allowlist de mensagens, sem propagar detalhes internos. Dezenove testes e a suíte completa foram aprovados no CI 35132873065, commit 5576eca.

O repositório de observações também oferece leitura para recuperação, exigindo a combinação completa de UUID da observação, UUID do produto e proprietário Telegram. UUID isolado, produto diferente ou outro proprietário retornam ausência; entradas estruturalmente inválidas são recusadas antes do banco. O retorno recompõe o mesmo contrato `PriceObservation`, incluindo falha explícita sem preço. Dez integrações PostgreSQL do repositório foram aprovadas no CI 35133476377, commit 93bc1ff. Essa porta ainda não está ligada ao worker.

`PostgreSQLTelegramVerificationResults` cria a fronteira para manter a coleta fora da transação da inbox. `find_for_claim` abre uma transação curta, bloqueia e valida o claim/payload e recupera eventual resposta imutável. Depois da coleta ou recuperação da observação, `save_for_claim` abre outra transação curta, revalida o mesmo claim e grava ou reutiliza a resposta. Lease expirado, identidade/chat/texto divergente e resposta conflitante não alteram o snapshot. Oito integrações PostgreSQL foram aprovadas no CI 35135283856, commit ffb43a0. O orquestrador ainda não foi ligado a essas portas.

`VerifyTelegramProduct` liga as portas sem manter transação durante a rede. Depois de validar a sintaxe, consulta resposta durável, observação escopada e somente então executa `VerifyProduct`. Sucesso e falha de coleta são formatados a partir da observação; produto ausente recebe resposta pública genérica. Se o lease expirar após a observação e antes do checkpoint, o retry encontra o snapshot e não chama o coletor novamente. Vinte e nove testes de aplicação e duas integrações PostgreSQL de replay/retomada foram aprovados no CI 35136457017, commit e48e151.

O worker possui agora a composição completa com `MercadoLivreCollector` por padrão e porta de coletor injetável para testes. O dispatcher encaminha `/verificar...` com update, lease, proprietário, chat, texto e horário; envio e conclusão da inbox permanecem depois do checkpoint durável. Dezessete testes do worker e três integrações PostgreSQL do fluxo composto/recuperação foram aprovados no CI 35162870668, commit a5e9406.

A admissão HTTP aceita o limite de comando `/verificar` com espaço/tabulação ou sem argumento; o segundo caso é persistido para produzir orientação durável. `/produtos` exibe o UUID completo somente junto aos produtos do proprietário. `/start`, `/ajuda` e mensagens de cadastro anunciam a consulta manual e mantêm alertas automáticos como pendentes. A suíte completa e o CI 35163551568 foram aprovados no commit cc427f0; deploy e aceitação manual ainda estão pendentes.

Coleta manual persistida e `/verificar` estão implementados e aprovados no CI, mas ainda aguardam deploy e aceitação manual. Job periódico e alertas continuam pendentes. A aceitação funcional entre duas contas da V2.9 foi concluída por relato do usuário; não houve consulta administrativa final.

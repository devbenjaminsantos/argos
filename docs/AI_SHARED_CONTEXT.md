...
- O webhook persiste cada comando suportado e autenticado na inbox antes de responder `200`; updates repetidos recebem `200` sem nova linha. Ausência da inbox ou falha SQL retorna `503`. `/start` e `/ajuda` já foram executados contra o bot real; `/cancelar` também está implantado e validado sem rascunho. Também faltam os comandos de produtos, domínio V2 e coletor Python.
- PostgreSQL usa SQLAlchemy, Alembic e psycopg. O projeto Supabase Argos existe em `sa-east-1` (`mkpziasjjfnwvtvuorig`), está saudável e tem a revisão Alembic `20260913_06` aplicada. `telegram_update_inbox` preserva o payload e modela disponibilidade, tentativas, lease, conclusão e dead letter. `telegram_users` separa propriedade e destino; `telegram_conversation_drafts` mantém o estado incompleto por proprietário. Os repositórios tratam deduplicação, concorrência, atualização monotônica do destino e cancelamento atômico do rascunho.
...
1. Implementar a persistência atômica do início de cadastro e da resposta por update. Aplicação `BeginTelegramRegistration` e porta `TelegramRegistrationRepository` estão preparadas, com duração inicial de 15 minutos, `/start` prévio, preservação de operação ativa e resultado original reutilizado. Lease e identidade devem ser verificados contra a inbox na transação. Ainda não há adaptador, tabela de resultados ou comando liberado. `20260913_06` foi aplicada pelo workflow `34788321746` no commit `75b31bf`; consulta independente confirmou versão UUID NOT NULL, default `gen_random_uuid()`, ownership de `argos_migrator`, SELECT/INSERT/UPDATE/DELETE do runtime e nenhuma leitura por `anon`, `authenticated` ou `service_role`. O início e avanço persistentes passaram no CI `34787786771`, commit `5099b39`, incluindo expiração, concorrência e rejeição de versão anterior ao cancelamento/recriação no mesmo timestamp. `begin` preserva ativos e substitui expirados; `advance` compara versão, proprietário, estado e expiração, recusando snapshots obsoletos. O grafo de transições está no domínio. Ainda faltam integração de `/adicionar`, validação das entradas e idempotência dos efeitos por update. A admissão está implantada e aceita em produção (deploy `dep-dajg54vqj5pc73dgitfg`, revisão `20260912_05`). Em 13/09/2026, dez comandos da rajada foram admitidos e concluídos em uma tentativa; o 11º ficou `rate_limited` sem inbox; um envio vários minutos depois foi admitido e concluído. A fronteira exata de 60 segundos e concorrência permanecem comprovadas no CI. Detalhes em `docs/V2_TELEGRAM_ADMISSION.md`. Confirmar em paralelo o menu `/cancelar` no BotFather.

...
1. Validar o teto de quota em janela controlada e a liberação após 60 segundos. Em 13/09/2026, a mensagem real `/ajuda` gerou decisão `admitted`, inbox `completed` e `attempt_count=1`; o usuário confirmou a ajuda com os três comandos. A integração está implantada: deploy `dep-dajg54vqj5pc73dgitfg`, commit `2dae74b`, live em 13/09/2026; readiness 200, sem erros recentes e tabelas de admissão inicialmente vazias. O CI `34777851672` aprovou o commit `3c28e40`, incluindo a rajada HTTP no PostgreSQL real. O código agora usa a admissão atômica na entrada e notifica o worker apenas para novos comandos admitidos. `20260912_05` foi aplicada pelo workflow `34776918057`, concluído com sucesso no commit `fdeb17f`; em 13/09/2026, a consulta independente confirmou ambas as tabelas vazias sob ownership de `argos_migrator`, runtime com SELECT/INSERT/UPDATE sem DELETE e roles públicas sem leitura. A migração e o adaptador já passaram em PostgreSQL 17 no CI `34705698375`, commit `64cb313`, com 120 testes, incluindo concorrência, rollback e downgrade seguro. Política e contrato isolados já existem: 10 comandos por usuário em janela móvel de 60 segundos, duplicatas sem quota e excedentes deduplicados sem execução ou resposta. O código de admissão está ativo em produção, mas a mensagem real foi validada e a aceitação do teto de quota ainda está pendente; critérios em `docs/V2_TELEGRAM_ADMISSION.md`. Confirmar em paralelo o menu `/cancelar` no BotFather.

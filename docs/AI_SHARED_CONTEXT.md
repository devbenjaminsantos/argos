...
1. Implementar o adaptador PostgreSQL do recebimento da URL. Validador sintático `domain/mercado_livre_url.py`, porta `TelegramRegistrationURLRepository` e caso de uso `ReceiveTelegramRegistrationURL` preparados e testados isoladamente. Nenhuma admissão de texto livre, migração ou mudança de produção neste incremento. Contrato: definir o contrato antes de admitir texto livre, preservar escopo por proprietário, expiração e versão, e persistir avanço e resposta atomicamente por update/lease. Não coletar a página nem concluir o produto neste próximo incremento.

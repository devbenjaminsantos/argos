...
1. Implementar transições persistentes e expiração dos rascunhos antes de integrar `/adicionar`. A admissão está implantada e aceita em produção (deploy `dep-dajg54vqj5pc73dgitfg`, revisão `20260912_05`). Em 13/09/2026, dez comandos da rajada foram admitidos e concluídos em uma tentativa; o 11º ficou `rate_limited` sem inbox; um envio vários minutos depois foi admitido e concluído. A fronteira exata de 60 segundos e concorrência permanecem comprovadas no CI. Detalhes em `docs/V2_TELEGRAM_ADMISSION.md`. Confirmar em paralelo o menu `/cancelar` no BotFather.

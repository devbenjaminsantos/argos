...
1. Integrar apelido ao fluxo de texto por estado após CI. PostgreSQLTelegramRegistrationAliasRepository implementado, usando tabela de resultados existente sem migração; testes preparados para repetição, concorrência, cancelamento, lease, expiração e rollback. CI pendente. A seleção do passo deve ocorrer na transação após consulta ao resultado por update, para evitar reprocessar a mesma entrada em outro estado. `normalize_product_alias`, `ReceiveTelegramRegistrationAlias` e `TelegramRegistrationAliasRepository` preparados: 1–60 caracteres após normalização NFC/espaços, sem controles ou links; awaiting_alias → awaiting_target_price, preservando URL e expires_at, nova versão e resposta durável por update/lease. Composição ainda pendente, sem mudança pública. A URL /up/MLBU foi aceita pelo usuário; consulta independente confirmou um rascunho awaiting_alias, fragmento removido, duração original de 15 minutos e update completed em uma tentativa. Deploy dep-dajuqnqd0e5s73dqpkp0, commit 162c26c, live; CI da correção 34842234274 aprovado. Nova URL após avanço e cancelamento da sequência URL permanecem pendentes de aceitação manual; cobertos no CI. Nenhuma migração ou operação de escrita no banco nesta etapa.

2. `/adicionar` está implantado e aceito no bot real. Deploy manual `dep-dajk7sh594qs73ch8spg`, commit `9a23c79`, live em 14/09/2026 UTC (13/09 no Brasil). Usuário confirmou `/ajuda`, início, operação já ativa, cancelamento e novo início. Consulta independente encontrou os cinco comandos completed em uma tentativa, dois resultados started, um already_active e um único rascunho awaiting_url vazio com duração de 15 minutos. CI `34789845853` aprovado no commit `724fe24`, com recuperação preservando resposta, versão e expiração. Banco em `20260913_07`; runtime com SELECT/INSERT nos resultados, sem UPDATE/DELETE, e sem leitura pública. Recebimento da URL implantado e aceito; cadastro completo continua pendente. O menu do BotFather continua pendente de confirmação.

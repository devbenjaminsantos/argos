...
- `postgres` continua membro administrativo dos dois grupos para operar objetos pelo provedor. Não reutilizar `postgres`, `service_role` ou o migrador como runtime HTTP. A identidade de migração e sua futura configuração permanecem fora do serviço `argos-api`.
...
1. Escolher o executor administrativo, rotacionar a senha de `argos_migrator_login` diretamente para seu secret store e configurar `ARGOS_MIGRATION_DATABASE_URL` somente nele.
2. Fazer o executor assumir `argos_migrator` e validar uma migração estrutural reversível em ambiente controlado.

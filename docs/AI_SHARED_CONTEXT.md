...
1. Criar uma identidade `LOGIN` dedicada de migração, associá-la ao grupo `argos_migrator` existente e validar uma migração estrutural em ambiente controlado.
2. Manter essa credencial apenas no executor administrativo escolhido, fora do runtime HTTP.

...
- V2 processa comandos suportados, persiste usuários, inbox, rascunhos e resultados de início. Produtos, observações e alertas ainda não estão implementados.
...
1. Integrar recebimento da URL ao worker e webhook após CI do adaptador. `PostgreSQLTelegramRegistrationURLRepository` implementado: bloqueia inbox, usuário e rascunho, confere texto e normalização, grava avanço e resposta em telegram_registration_results existente e preserva expiração. Nenhuma migração necessária. Testes de concorrência, cancelamento, expiração, rollback e recuperação preparados; CI `34836132177` aprovado no commit `3316bd6`, com PostgreSQL 17. Validador sintático `domain/mercado_livre_url.py`, porta `TelegramRegistrationURLRepository` e caso de uso `ReceiveTelegramRegistrationURL` preparados e testados isoladamente. Nenhuma admissão de texto livre ou mudança de produção neste incremento. Contrato: definir o contrato antes de admitir texto livre, preservar escopo por proprietário, expiração e versão, e persistir avanço e resposta atomicamente por update/lease. Não coletar a página nem concluir o produto neste próximo incremento.
2. `/adicionar` está implantado e aceito no bot real. Deploy manual `dep-dajk7sh594qs73ch8spg`, commit `9a23c79`, live em 14/09/2026 UTC (13/09 no Brasil). Usuário confirmou `/ajuda`, início, operação já ativa, cancelamento e novo início. Consulta independente encontrou os cinco comandos completed em uma tentativa, dois resultados started, um already_active e um único rascunho awaiting_url vazio com duração de 15 minutos. CI `34789845853` aprovado no commit `724fe24`, com recuperação preservando resposta, versão e expiração. Banco em `20260913_07`; runtime com SELECT/INSERT nos resultados, sem UPDATE/DELETE, e sem leitura pública. Recebimento da URL e cadastro completo ainda não estão liberados. O menu do BotFather continua pendente de confirmação.

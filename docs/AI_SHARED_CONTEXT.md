...
- V2 processa comandos suportados, persiste usuários, inbox, rascunhos e resultados de início. Cadastro confirmado de produtos está implementado e aceito; consulta, remoção, observações e alertas continuam pendentes.
...
1. Implementar `/produtos` em incremento isolado, com leitura escopada ao proprietário, lista vazia e apresentação dos dados cadastrados; sem iniciar coleta ou alertas. Cadastro completo aceito manualmente em 14/09/2026. Consulta independente encontrou dois resumos, uma correção, um produto e duas respostas sem cadastro ativo; confirmação completed em uma tentativa, inbox com 70 completed e nenhum outro estado. Novo rascunho awaiting_interval criado depois da confirmação foi preservado. Deploy atual dep-dak2oje743jc73fojcig, af6068d, live; resultados da confirmação antecedem esse redeploy. Diff c2495c7..af6068d contém somente documentação. CI 34865934346 aprovou integração HTTP→worker→PostgreSQL 17. Migração 20260914_08 aplicada via workflow 34864818265; runtime possui SELECT/INSERT nos produtos, sem UPDATE/DELETE/TRUNCATE. Correção mantém expiração; confirmação cria produto, consome rascunho e grava resposta atomicamente, com slots 1..3 e chave única por proprietário. Coleta, alertas e remoção permanecem pendentes.
